...
import { Edit2, MessageSquare, AlertTriangle, Clock, PlusCircle, Headphones } from 'lucide-react'
...
interface PodcastType {
  ...
  media_type: 'Podcast';
  genre?: 'News' | 'Comedy' | 'Education' | 'Business' | null;
...
export default function PodcastList({ items, onEdit, onLike, onCreate }: PodcastListProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-grow overflow-auto bg-white rounded-lg shadow-md p-4">
        {items.length === 0 ? (
          <Empty description="No podcasts found" />
        ) : (
                ...
                <div className="flex items-center p-4">
                  ...
                  <div className="flex-grow">
                    <div className="flex items-center space-x-2 mb-1">
                      <h3 className="text-lg font-semibold text-gray-800">{item.name || 'Untitled Podcast'}</h3>
                      <span className={`text-xs font-medium px-2 py-1 rounded-full ${getStatusColor(item.status)}`}>
                        {item.status}
                      </span>
                    </div>
                    <p className="text-sm text-gray-600">{item.artist_name || 'Unknown Host'}</p>
                  ...
                  <div className="flex flex-col items-end space-y-2">
                    <div className="flex items-center space-x-4">
                      ...
                      <span className="text-sm text-gray-500" title="Total Listens">
