...
const MediaForm: React.FC<{ mediaId?: number }> = ({ mediaId }) => {
  ...
  const fetchMediaDetails = async () => {
    ...
    try {
      setLoading(true)
      const mediaData = await mediaService.getMediaDetail(mediaId)
      setMedia(mediaData.data)
      // Convert price from cents to dollars
      const formData = {
        ...mediaData.data,
        price: mediaData.data.price ? mediaData.data.price / 100 : undefined
      }
      form.setFieldsValue(formData)
      setLyric(mediaData.data.lyric || '')
    } catch (error: any) {
      handleError(error)
    } finally {
      setLoading(false)
  ...
  const handleSave = async (values: MediaType) => {
    ...
    try {
      ...
      Object.keys(values).forEach(key => {
        if (values[key] !== undefined && values[key] !== null) {
          if (key === 'img_url' || key === 'audio_url') {
            ...
            const priceInCents = Math.round(parseFloat(values[key]) * 100)
      ...
      if (mediaId) {
        ...
        //   audioSrc: values.audio_url && values.audio_url.file ? URL.createObjectURL(values.audio_url.file) : media?.audio_url,
        //   coverArt: values.img_url && values.img_url.file ? URL.createObjectURL(values.img_url.file) : media?.img_url,
        // })
      } else {
        ...
        addTrack({
          ...
          title: values.name,
          artist: values.artist_name,
          audioSrc: values.audio_url && values.audio_url.file ? URL.createObjectURL(values.audio_url.file) : '',
          coverArt: values.img_url && values.img_url.file ? URL.createObjectURL(values.img_url.file) : '',
        })
      }
    } catch (error: any) {
      handleError(error)
  ...
  const handleTranscribe = async () => {
    ...
    // if (!['User Vip', 'Admin', 'Staff'].includes(user?.role || '')) {
