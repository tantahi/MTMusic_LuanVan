...
export default function PaymentReceiptsPage() {
  ...
  useEffect(() => {
    const fetchReceipts = async () => {
      ...
      try {
        ...
        message.error(error instanceof Error ? error.message : 'An error occurred while fetching receipts')
      } finally {
        setLoading(false)
      }
    }
  ...
  const formatCurrency = (value: number, itemType: string) => {
  ...
  const columns: ColumnsType<PaymentReceipt> = [
    ...
    },
    {
      ...
      dataIndex: 'price',
      key: 'price',
      render: (price: number, record: PaymentReceipt) => formatCurrency(price, record.item_type),
    },
    {
      title: 'Seller',
      dataIndex: ['Seller', 'full_name'],
      key: 'seller',
      render: (seller: string, record: PaymentReceipt) => 
        record.item_type === 'VIP Subscription' ? 'System' : seller,
    },
    {
      title: 'Date',
